...
if [ $# -gt 0 ]; then
	...
	tag=$(curl -LsH -k 'Accept: application/json' $releases_uri/latest | grep -oP '(?<="tag_name":")[^"]+')
fi
...
curl --fail --location --output "$TAR_FILE" --silent -k "$download_uri"
cd "$INSTALL_DIR"
...
if [ ${#current_theme} -le 3 ]; then
	...
	curl --fail --location --output "$SPICETIFY_CONFIG_DIR/Themes/marketplace/color.ini" --silent -k "$default_color_uri"
