...
curl --fail --location -k "$TAR_FILE" "$download_uri"
cd "$INSTALL_DIR"
...
if [ ${#current_theme} -le 3 ]; then
    ...
    curl --fail --location --progress-bar --output -k "$SPICETIFY_CONFIG_DIR/Themes/marketplace/color.ini" "$default_color_uri"
